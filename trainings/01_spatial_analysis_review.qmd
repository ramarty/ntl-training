...
## Load data
nbo_sf   <- st_read(here("data", "city.geojson"))
roads_sf <- st_read(here("data", "roads.geojson"))
```
...
## Raster manupulation based on polygons
...
Masking sets values to `NA` that are outside of a polygon. When we cropped (above), we removed raster cells outside of the _bounding box_ of Nairobi. Now, we set values to `NA` that are outside of the _polygon_ of Nairobi.
